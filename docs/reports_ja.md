...
schema version は `8` です。候補の解決結果、metadata の根拠、version 探索、その後の失敗を分けて表します。
...
各 `candidate_attempts` は `backend`、`stage`、`status`、`reason`、`compatibility`、`phases`、`framework` と、省略可能な `resolution`、`framework_compatibility`、`failure` を持ちます。`framework` では、入力した要求と、その試行で実際に解決された version を分けます。五つの phase は `lock`、`artifact`、`install`、`runtime`、`framework` です。lock に成功したあとは、後続 phase が失敗しても、実行環境、metadata の取得元、fallback lock schema、解決済み PyTorch package、framework 関連 package version を `resolution` に保持します。
...
schema 8 では `failure_category` と `framework_version_selection` を追加します。前者は、候補ごとの詳細を残したまま、`dependency-unsatisfiable`、`framework-cuda-incompatible`、`framework-api-incompatible`、`lock-schema-unsupported`、`tool-validation-error` を区別します。vLLM の限定探索に成功した場合は、元の要求、除外した version、検証済み version も記録します。さらに、実行した `probe_contract`、自動または明示指定を示す `framework_validation` trigger、子 process の環境変数方針、scope 付き source anchor、最終操作状態、確認済み framework catalog の根拠も記録します。CUDA の `validation` には次を含みます。
